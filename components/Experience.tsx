...
export default function Experience() {
  ...
  return (
    <section id="experience" ref={ref} className="px-6 md:px-8 py-16 md:py-24" style={{ background: 'var(--bg-2)' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {experiences.map((exp, idx) => (
                <div key={exp.id} className="reveal flex gap-4 md:gap-7" style={{ paddingBottom: idx < experiences.length - 1 ? 28 : 0 }}>
                  ...
                  <div className="card" style={{ flex: 1, padding: exp.highlights.length === 0 ? '14px 20px' : '24px', opacity: exp.highlights.length === 0 ? 0.6 : 1 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: exp.highlights.length > 0 ? 12 : 0 }}>
                      <div>
                        ...
                        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 2, wordBreak: 'keep-all' }}>{exp.role}</h3>
                    ...
                    {exp.highlights.length > 0 && (
                      <ul style={{ display: 'flex', flexDirection: 'column', gap: 6, listStyle: 'none' }}>
                        {exp.highlights.map((item, i) => (
                          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, wordBreak: 'keep-all' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {education.map((edu, i) => (
                  <div key={i} style={{ borderLeft: '2px solid var(--accent)', paddingLeft: 12 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2, wordBreak: 'keep-all' }}>{edu.school}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {profile.differentiators.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', wordBreak: 'keep-all' }}>{s.title}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', wordBreak: 'keep-all' }}>{s.desc.slice(0, 30)}...</div>
                    </div>
                  </div>
                ))}
              </div>
