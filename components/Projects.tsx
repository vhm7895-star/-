...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={ref} style={{ padding: '100px 32px', background: 'var(--bg)' }}>
            ...
            return (
              <div key={project.id} className="reveal card" style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 19, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.3, marginBottom: 6 }}>{project.title}</h3>
                  <div style={{ fontFamily: 'var(--font-ko)', fontSize: 12, color: 'var(--text-secondary)' }}>{project.company} · 기여도 {project.contribution}</div>
                </div>

                <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.75 }}>{project.problem}</p>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {project.strategies.map((s, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                    ...
                    {project.results.map((r, i) => {
                      ...
                      return (
                        <div key={i}>
                          ...
                          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-secondary)', marginTop: 4 }}>{r.note}</div>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div style={{ borderLeft: '2px solid var(--accent)', paddingLeft: 14 }}>
                  ...
                  <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7, fontStyle: 'italic' }}>"{project.insight}"</p>
                </div>
              </div>
            )
          })}
